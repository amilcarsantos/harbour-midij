...
bool MidiMappingGui::loadGraphicItems(QDomElement root, QList<QGraphicsItem*>& guiItems)
{
	...
	while (!image.isNull())
	{
		...
		QPixmap pixmap = pixmapMap.load(name, file);
		if (pixmap.isNull())
		{
...
void MidiMappingGui::midiShortMsgIn(MidiMessage inputCommand, char midivalue)
{
	// check mappings
	...
	qWarning(QString("MidiMappingGui: UNHANDLED MIDI MSG >>>>>> status:0x%1; ch:%2; no:0x%3; value:0x%4")
		.arg(inputCommand.getMidiStatusByte() & 0xF0,2,16)
		.arg(inputCommand.getMidiStatusByte() & 0xF,2,16)
		.arg(inputCommand.getMidiNo(),2,16)
		.arg((int)midivalue,2,16).toLocal8Bit());
}
...
void MidiMappingGui::midiShortMsgIn(uchar statusByte, uchar midino, uchar midivalue)
{
	...
	midiShortMsgIn(inputCommand, (char)midivalue);
}
